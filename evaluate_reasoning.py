...
def evaluate(model, tokenizer, dataset_path, generate_response_func, max_new_tokens=2048, model_name=""):
    ...
    def generate_response(query, context):
        messages = [{"role": "system",
                     "content": "你是一位经验丰富的临床医学专家，具备深厚的专业知识和丰富的临床经验，能够基于患者描述的症状和病史，提供科学、合理的医疗建议"},
...
def train_reasoning(model_path, model_name):
    ...
    evaluate(model, tokenizer, generate_response_func='generate_response_reasoning',
             dataset_path='./qa-reasoning-test/qa_reasoning_test.csv', max_new_tokens=4096,
             model_name="7b-instruct-think-reasoning")
...
def print_thinking(model, tokenizer, max_new_tokens=4096):
    messages = [{"role": "system",
                 "content": """你是一位经验丰富的临床医学专家，具备深厚的专业知识和丰富的临床经验，能够从患者提供的症状和病史入手，进行系统性评估和缜密推理。通过对病情的逐步剖析，最终为患者提供科学、合理且个性化的医疗建议。"""},
                {"role": "user",
                 "content": "一位68岁的女性患者有15年的糖尿病病史，近两个月来一直在接受胰岛素强化治疗，但她的空腹血糖仍然高达10mmol/L。在这种情况下，凌晨3点的血糖检测是否是最具临床治疗指导意义的检查呢？"}]
    inputs = tokenizer.apply_chat_template(
        messages,
        tokenize=True,
        add_generation_prompt=True,
...
def evaluate_reasoning(model_path="unsloth/Qwen2.5-7B-Instruct"):
    model, tokenizer = FastLanguageModel.from_pretrained(
    ...
    res = evaluate(model, tokenizer, generate_response_func='generate_response_reasoning',
                   dataset_path='./qa-reasoning/qa_reasoning_all.csv', max_new_tokens=4096,
                   model_name=model_path.split("/")[-1])
    print(res)


evaluate_reasoning()
evaluate_reasoning("/root/autodl-tmp/unsloth/Qwen2.5-7B-Instruct_sft1")
evaluate_reasoning("/root/autodl-tmp/unsloth/Qwen2.5-7B-Instruct_sft2")
evaluate_reasoning("/root/autodl-tmp/unsloth/Qwen2.5-7B-Instruct_sft2_without_sft1")
evaluate_reasoning("/root/autodl-tmp/unsloth/Qwen2.5-7B-Instruct_think_reasoning")