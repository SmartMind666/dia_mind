...
def get_sft1_train_dataset(tokenizer):
  ...
  def qa_to_conversation(example):
      conversation = [
          {"role":"system", "content":"你是一位经验丰富的临床医学专家，具备深厚的专业知识和丰富的临床经验，能够基于患者描述的症状和病史，提供科学、合理的医疗建议"},
  ...
  def formatting_prompts_func(examples):
      ...
      for convo in convos:
          try: 
              texts.append(tokenizer.apply_chat_template(convo, tokenize=False))
...
def get_reasoning_train_dataset(tokenizer):
  ...
  def qa_to_conversation(example):
      conversation = [
          {"role":"system", "content":"你是一位经验丰富的临床医学专家，具备深厚的专业知识和丰富的临床经验，能够从患者提供的症状和病史入手，进行系统性评估和缜密推理。通过对病情的逐步剖析，，最终为患者提供科学、合理且个性化的医疗建议"},
          {"role": "user", "content": example["query"]},
...
def get_reasoning_rag_train_dataset(tokenizer):
  ...
  def qa_to_conversation(example):
      conversation = [
          {"role": "system", "content": f"""你是一位经验丰富的临床医学专家，具备深厚的专业知识和丰富的临床经验，能够从患者提供的症状和病史入手，结合提供的资料和自身知识，进行系统性评估和缜密推理。通过对病情的逐步剖析，，最终为患者提供科学、合理且个性化的医疗建议。以下是可供参考的资料：\n{example["context"]}"""},
          {"role": "user", "content": example["query"]},
          {"role": "assistant", "content": f"<think>\n{example['thinking']}\n</think>\n{example['answer']}"},
      ]
      return {"conversations": conversation}
...
def evaluate(model, tokenizer, dataset_path, generate_response_func, max_new_tokens=2048, model_name=""):
  ...
  # dataset = dataset.train_test_split(test_size=2, seed=3834)
  ...
  def generate_response_reasoning(query, context):
    messages = [{"role":"system", "content":"你是一位经验丰富的临床医学专家，具备深厚的专业知识和丰富的临床经验，能够从患者提供的症状和病史入手，进行系统性评估和缜密推理。通过对病情的逐步剖析，，最终为患者提供科学、合理且个性化的医疗建议"},{"role": "user", "content": query}]
    inputs = tokenizer.apply_chat_template(
        messages,
        tokenize=True,
        add_generation_prompt=True,
  ...
  def generate_response_reasoning_rag(query, context):
    messages = [{"role": "system", "content": f"""你是一位经验丰富的临床医学专家，具备深厚的专业知识和丰富的临床经验，能够从患者提供的症状和病史入手，结合提供的资料和自身知识，进行系统性评估和缜密推理。通过对病情的逐步剖析，，最终为患者提供科学、合理且个性化的医疗建议。以下是可供参考的资料：\n{context}"""}, {"role": "user", "content": query}]
    inputs = tokenizer.apply_chat_template(
        messages,
        tokenize=True,
        add_generation_prompt=True,
...
def train_sft1(model_path, model_name):
  ...
  evaluate(model, tokenizer, generate_response_func='generate_response', dataset_path='./qa_test/qa_test.csv', model_name=model_name)
...
def train_sft2(model_path, model_name):
  ...
  model.save_pretrained_merged(
    ...
    tokenizer=tokenizer,
    save_method="merged_16bit")
  evaluate(model, tokenizer, generate_response_func='generate_response', dataset_path='./diabetes_qa_test/diabetes_qa_test.csv', model_name=model_name)
...
def train_reasoning(model_path, model_name):
  ...
  model.save_pretrained_merged(
    ...
    tokenizer=tokenizer,
    save_method="merged_16bit")
  evaluate(model, tokenizer, generate_response_func='generate_response_reasoning', dataset_path='./qa-reasoning/qa_reasoning_all.csv', max_new_tokens=5120, model_name=model_name)
...
def train_rag_reasoning(model_path, model_name):
  ...
  model.save_pretrained_merged(
    ...
    tokenizer=tokenizer,
    save_method="merged_16bit")
  evaluate(model, tokenizer, generate_response_func='generate_response_reasoning_rag', dataset_path='./qa-reasoning-rag/qa_rag_reasoning_all.csv', max_new_tokens=5120, model_name=model_name)
...
load_in_4bit = True
model, tokenizer = FastLanguageModel.from_pretrained(
  model_name = f"/root/autodl-tmp/unsloth/Qwen2.5-7B-Instruct",
  max_seq_length = max_seq_length,
  dtype = dtype
)
...
res = evaluate(model, tokenizer, generate_response_func='generate_response', dataset_path='./qa_test/qa_test.csv', model_name="7b-original")
