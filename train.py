...
def evaluate(model, tokenizer, dataset_path, generate_response_func, max_new_tokens=2048, model_name=""):
    ...
    dataset = test_dataset.train_test_split(test_size=500, seed=3834)
    ...
    def generate_response_without_instrcut(query, context):
        messages = f"<|im_start|>user\n{query}<|im_end|>\n<|im_start|>assistant\n"
        inputs = tokenizer(
            [messages],
            return_tensors="pt",
...
def train_sft2(model_path, model_name):
    ...
    res = evaluate(model, tokenizer, generate_response_func='generate_response',
             dataset_path='./diabetes_qa_test/diabetes_qa_test.csv', model_name=f"{model_name}_sf2")
    print(res)
    model.save_pretrained_merged(
...
def e1():
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name="/root/autodl-tmp/unsloth/Qwen2.5-7B",
        max_seq_length=8092,
        dtype=None,
    ...
    res = evaluate(model, tokenizer, generate_response_func='generate_response', dataset_path='./qa_test/qa_test.csv',
             model_name="Qwen2.5-7B_ori_esft1")
    print(res)
def e2():
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name="/root/autodl-tmp/unsloth/Qwen2.5-7B",
    ...
    res = evaluate(model, tokenizer, generate_response_func='generate_response', dataset_path='./diabetes_qa_test/diabetes_qa_test.csv',
             model_name="Qwen2.5-7B_ori_esft2")
    print(res)
# e1()
# e2()
# train_sft1("/root/autodl-tmp/unsloth", "Qwen2.5-7B")
train_sft2("/root/autodl-tmp/unsloth", "Qwen2.5-7B")